...
> このリポジトリで見せたいのは、照合の仕組み・データ設計・テスト・AWS アーキテクチャです。
...
> 実運用中のシステムを題材に「構築(AWS 3 層構成 + キャッシュ + 監視)」と
> 「運用(数ヶ月の稼働・乖離検知・障害対応)」の両方を 1 プロジェクトで示すことを狙っている。
...
| シグナル→約定の実行率 | 実行率 < 90% で要確認 / < 50% で NG | バックテストは全約定前提。失注分だけ想定取引を取りこぼしている |
...
| 同時建玉数の上限 | コードの `max_positions=6` に到達で要確認 | ドキュメント方針「上限なし」との食い違い(既知) |
| 手法別リスク% の一致 | `baseline` の値と不一致で要確認 | k 値・リスク設定のズレ |
| 成績 vs バックテスト | 判定せず並記(運用日数を明示) | サンプルが溜まったら CAGR / MaxDD を目安比較 |
...
### この冗長化構成は意図的なオーバースペック

個人の売買ボット 1 台の監視に ALB + Auto Scaling + ElastiCache は実運用としては過剰。
**実務で必要になる設計パターン(冗長化・水平スケール・キャッシュ戦略・ヘルスチェック設計・
IaC 化)を、実際に動いているシステムを題材に一通り手を動かして学ぶことが目的**。
コスト・複雑性とのトレードオフ判断自体も設計の一部と考えており、
検証後は不要なリソースを削除して単一構成に戻す前提。
