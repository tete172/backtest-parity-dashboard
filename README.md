...
| **二重発注 / 二重稼働** | 「複数positionId検出」8 件。ログの二重出力から疑っていた 2026-09 時点の二重稼働再発を実機調査で**確認・解消済み(2026-09-13)**: `fxbot.service` とは別名の `squeeze-bot.service`(第2 systemd サービス)が同じスクリプトを独立に動かし続けており、5 分おきの cron 監視で自動復活する構成になっていた。停止・無効化・cron 削除で解消 |
| **同時建玉数** | 最大 5(コードの上限 6 未満)。方針の懸念どおりだが、この期間は上限に達していない |
| **有効証拠金** | 3 ヶ月の観測期間中、0 円のまま **10.7 日間**動けなかった局面が 1 回(1時間毎のスナップショットで延べ224時点。その後入金で復帰) |

> 上の表はいずれも `app/reconciliation.py` の自動チェックがダッシュボードに出す結果。
> 下記はそれとは別に、**生ログを直接読んで**見つけたもの(現時点ではダッシュボードのチェックに未実装)。

**DM×PSAR のトレールSLが実運用では1度も成功していない**(2026-09-07):

```
[DM トレイル発動]   × 309 回   ← 全て USD_JPY の同一ポジション
[新SL逆指値登録]     ×   0 回   ← 成功ゼロ
[新SL逆指値失敗]     × 309 回   ← ERR-200 "settlement quantity exceeds the settable quantity"
```

旧SLはキャンセル成功 → 新SL発注は毎回失敗、を **5.5 時間・毎分リトライ**していた。つまりその間
このポジションは **ストップロス無しで晒されていた**。3 ヶ月ログ中トレール起動条件(+1R到達)に
届いたのはこの1件だけで、その1件が100%失敗 = **実運用でDMのトレールが機能した実績はゼロ**。
ERR-200(建玉数量と決済可能数量の不一致)は、上表の「二重発注/複数positionId検出」と根が同じ
可能性が高い。→ ログに `[DM トレイル発動]` / `[新SL逆指値失敗]` のパーサ対応を追加すれば
自動チェック化できる(`ingest/parser.py` 拡張・次の一手)。

> ✅ **対応済み(2026-09-13)**: 新SL発注時に古い記憶値ではなく実際の建玉サイズを使うよう修正、
> 失敗時は旧価格でSLを再発注(無防備回避)、連続失敗をアラートするよう監視対象ボット本体に反映。
> このデプロイ作業中に上の「二重稼働」も実機で再現・原因特定・解消している。
